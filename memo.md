...
- **需求**: 
  ...
  2. 设置放在最下方
  3. 样式改为⬅️侧边选项卡形式
  4. 个人点击后可以修改头像、昵称、职业、性别、年龄等信息，并且可以修改密码
  5. 设置点击后可以更换主题皮肤，设计多种简单大气的扁平化设计风格
- **实现方案**: 
  1. **侧边选项卡设计**: 创建60px宽的垂直选项卡栏，包含聊天、个人、设置三个选项
  2. **个人资料弹窗**: 包含头像上传、昵称、职业、性别、年龄、密码修改等字段
  3. **主题设置弹窗**: 提供4种主题选择（默认、深色、暖色、自然）
  4. **主题系统**: 实现完整的主题色彩配置，支持动态切换
- **技术特点**: 
  1. **响应式主题**: 所有UI元素都支持主题色彩动态切换
  2. **扁平化设计**: 采用现代扁平化设计风格，简洁大气
  3. **弹窗交互**: 使用Modal组件实现弹窗式交互体验
  4. **状态管理**: 使用React状态管理主题切换和弹窗显示
- **UI组件**: 
  1. **侧边选项卡**: 垂直布局，图标+文字，支持选中状态高亮
  2. **个人资料表单**: 包含输入框、选择器、按钮等表单元素
  3. **主题选择器**: 网格布局，每个主题显示预览和名称
  4. **弹窗系统**: 统一的弹窗样式和交互逻辑
- **主题配置**: 
  - **默认主题**: 蓝色系，iOS风格
  - **深色主题**: 黑色系，夜间模式
  - **暖色主题**: 橙色系，温暖风格
  - **自然主题**: 绿色系，清新风格
- **预期效果**: 提供更现代化的用户界面，支持个性化设置，提升用户体验 